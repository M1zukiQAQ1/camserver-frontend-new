...
const legacyProxyPrefix = '/api/backend/'

/**
 * Forwards /api/** to the Spring backend, preserving the API path.
 * The more specific /api/_nuxt_icon/:collection route remains owned by Nuxt.
 *
...
function buildTargetUrl(backendBase: string, requestUrl: URL) {
  ...
  // Keep links generated by older frontend builds working during a rollout.
  let forwardedPath = requestUrl.pathname.startsWith(legacyProxyPrefix)
    ? requestUrl.pathname.slice(legacyProxyPrefix.length - 1)
    : requestUrl.pathname

  // Spring still exposes camera settings at the legacy root path.
  if (forwardedPath === '/api/settings') {
    forwardedPath = '/settings'
  }
